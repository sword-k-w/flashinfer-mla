...
from run import validate_metadata

LTC = "lts__t_requests_srcunit_ltcfabric.sum"


def summarize_sector_only(root):
    ...
    for p in doc["profiles"]:
        ...
        records = [r for r in raw[i + 2 :] if r and r[0].strip()]
        assert len(records) == 1
        values = dict(zip(header, records[0], strict=True))
        ...
        assert set(p["metrics"]) == set(doc["metrics"])
        for name, metric in p["metrics"].items():
            assert memory.parse_number(values[name], name) == metric["value"]
            assert unit_map[name] == metric["unit"]
        command = json.loads(Path(p["command_file"]).read_text())
        argv = command["argv"]
        assert argv[argv.index("--metrics") + 1].split(",") == doc["metrics"]
        log = raw_path.with_suffix(".log").read_text()
        assert memory.parse_target_metadata(log) == p["target_metadata"]
        validate_metadata(p["target_metadata"], p["sq"], p["mode"])
        assert "POST_PROFILE_CORRECTNESS bitwise equal output and LSE" in log
        rate, total, hit, miss = [memory.metric_value(p, m) for m in doc["metrics"]]
        delta = (hit + miss) / total - 1
        assert p["l2"]["hit_rate_pct"] == rate
        assert p["l2"]["counter_sum_relative_error"] == delta
        usable = 0 <= rate <= 100 and abs(delta) <= 0.05
        assert p["l2"]["hit_rate_usable"] == usable
        row = dict(
            sq=p["sq"],
            mode=p["mode"],
            trial=p["trial"],
            passes=p["ncu_replay_passes"],
            rate=rate,
            total=total,
            hit=hit,
            miss=miss,
        ...
        rows.append(row)
        lines.append(
            f"| [{raw_path.name}]({raw_path.name}) | {p['ncu_replay_passes']} | {rate:.2f} | {total:.0f} | {hit:.0f} | {miss:.0f} | {100 * delta:+.2f} | {'passed' if usable else 'flagged'} |"
        )
        for path in (
            raw_path,
            raw_path.with_suffix(".log"),
            Path(p["report"]),
            Path(p["command_file"]),
        ):
    ...
    changes = [
        p for p, h in doc["source_sha256"].items() if memory.file_sha256(REPO / p) != h
    ]
    ...
    memory.write_json_atomic(
        ...
        dict(
            status="passed",
            profiles_verified=len(rows),
            source_changes=changes,
            quality_warning_count=sum(not r["usable"] for r in rows),
            files_sha256=files,
        ),
    )
...
def main():
    ...
    parser.add_argument("--sector-only-root", type=Path)
    args = parser.parse_args()
    if args.sector_only_root:
        summarize_sector_only(args.sector_only_root.resolve())
    ...
    for p in doc["profiles"]:
        ...
        argv = command["argv"]
        assert (
            argv[argv.index("--metrics") + 1].split(",")
            == doc["settings"]["groups"][p["group"]]
        )
        for flag, expected in (
            ...
            ("--launch-count", "1"),
            ("--seqlen-q", str(config["seqlen_q"])),
            ("--mode", p["mode"]),
        ):
            assert argv[argv.index(flag) + 1] == expected
        metadata = memory.parse_target_metadata(
            raw_path.with_suffix(".log").read_text()
        )
        assert metadata == p["target_metadata"]
        validate_metadata(metadata, config["seqlen_q"], p["mode"])
        assert (
            "POST_PROFILE_CORRECTNESS bitwise equal output and LSE"
            in raw_path.with_suffix(".log").read_text()
        )
        if p["group"] == "memory":
            old_l2 = p["l2"].copy()
    ...
    expected_keys = {
        ("prefill" if sq == 128 else "decode", sq, group, mode, trial)
        for sq in (1, 4, 128)
        for group in ("ltc", "memory")
        for mode in ("baseline", "compact")
        for trial in range(1, trials + 1)
    }
    assert keys == expected_keys
    for sq in (1, 4, 128):
        selected = [p for p in doc["profiles"] if p["config"]["seqlen_q"] == sq]
        reference = selected[0]["target_metadata"]
        for p in selected:
            for field in ("plan", "scale", "audits", "compact_physical_span"):
                assert p["target_metadata"][field] == reference[field]
    ...
    for workload, sq in (("decode", 1), ("decode", 4), ("prefill", 128)):
        ...
        for mode in ("baseline", "compact"):
            selected = [
                p
                for p in doc["profiles"]
                if p["config"]["workload"] == workload
                and p["config"]["seqlen_q"] == sq
                and p["mode"] == mode
            ]
            ...
            item["modes"][mode] = dict(
                ltc_requests=ltc,
                ltc_median=statistics.median(ltc),
                l2_hit_rate_pct=rates,
                l2_median=statistics.median(rates),
                l2_spread_pp=max(rates) - min(rates),
                l2_usable=[p["l2"]["hit_rate_usable"] for p in l2],
                ...
                if not p["l2"]["hit_rate_usable"]:
                    warnings.append(
                        dict(
                            ...
                            trial=p["trial"],
                            l2=p["l2"],
                            raw_csv=str(Path(p["raw_csv"]).relative_to(root)),
                        )
                    )
        a, b = (item["modes"][m] for m in ("baseline", "compact"))
        ...
        def rates(m):
            return ", ".join(
                f"{v:.2f}" + ("†" if not valid else "")
                for v, valid in zip(m["l2_hit_rate_pct"], m["l2_usable"], strict=True)
            )

        lines.append(
            f"| {workload} Sq={sq} | {traffic(a)} | {traffic(b)} | {reduction:.2f}% | {rates(a)} | {rates(b)} |"
        )
        summary.append(item)
    lines += [
        "",
    ...
    for p in doc["profiles"]:
        ...
        l2 = p["l2"]
        lines.append(
            f"| [{path}]({path}) | {l2['hit_rate_pct']:.2f} | {l2['total_sectors']:.0f} | {l2['lookup_hit_sectors']:.0f} | {l2['lookup_miss_sectors']:.0f} | {100 * l2['counter_sum_relative_error']:+.2f} |"
        )
    lines += [
        "",
    ...
    (root / "results.md").write_text("\n".join(lines))
    memory.write_json_atomic(
        root / "summary.json", dict(results=summary, l2_quality_warnings=warnings)
    )
    source_changes = [
        p
        for p, digest in doc["source_sha256"].items()
        if memory.file_sha256(REPO / p) != digest
    ]
    assert not source_changes, source_changes
    samples = [
        json.loads(s)
        for s in (root / "resource_samples.jsonl").read_text().splitlines()
    ]
    busy = []
    ...
    memory.write_json_atomic(
        ...
        dict(
            status="passed",
            profiles_verified=len(rows),
            l2_quality_warning_count=len(warnings),
            source_changes_since_start=source_changes,
            files_sha256=files,
            resources=dict(
                samples=len(samples),
                min_available_memory_gib=min(s["mem_available_kib"] for s in samples)
                / 2**20,
                peak_aggregate_rss_gib=max(
                    s["aggregate_process_rss_kib"] for s in samples
                )
                / 2**20,
                min_tmp_free_gib=min(s["tmp_disk_free_bytes"] for s in samples) / 2**30,
                peak_cpu_busy_pct=max(busy, default=0),
                peak_load_1m=max(s["load"][0] for s in samples),
            ),
        ),
    )
